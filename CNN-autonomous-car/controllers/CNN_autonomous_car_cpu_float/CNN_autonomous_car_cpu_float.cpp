            ...
            for (int kc = 0; kc < current_conv_layer.in_channels; kc++) {
              for (int kh = 0; kh < current_conv_layer.kernel_size; kh++) {
                for (int kw = 0; kw < current_conv_layer.kernel_size; kw++) {
                  const int prevIdx = kc * current_conv_layer.in_height *
                                          current_conv_layer.in_width +
                                      (kh + nh) * current_conv_layer.in_width +
                                      (kw + nw);
                  const int kernelIdx = kc * current_conv_layer.kernel_size *
                                            current_conv_layer.kernel_size +
                                        kh * current_conv_layer.kernel_size +
                                        kw;
                  sum += prevx[prevIdx] *
                         current_conv_layer.kernels[nc].weights[kernelIdx];
                }
              }
            }
