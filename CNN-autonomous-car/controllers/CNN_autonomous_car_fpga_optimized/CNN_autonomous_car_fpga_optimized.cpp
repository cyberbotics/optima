...
static fixed_point_t inputFixed[ConvNetwork_IC_CONV1 * ConvNetwork_HT_CONV1 *
                                ConvNetwork_WT_CONV1];
...
static double beginEndAccTime = 0;
static double midStepAccTime = 0;
...
void set_dfe_actions(ConvNetwork_actions_t *actions) {
  ...
  actions->instream_inputfromcpu = &inputFixed[0];
...
int main(int argc, const char *argv[]) {
  ...
  do {
    const double *carPosition = NULL; // wb_supervisor_node_get_position(car);

    gettimeofday(&midStep, NULL);
    ...
    struct timeval cnnStart;
    gettimeofday(&cnnStart, NULL);
    ...
    struct timeval cnnEnd;
    gettimeofday(&cnnEnd, NULL);
    float cnn_time = elapsed_time(cnnStart, cnnEnd);
    // printf("Run time = %f\n", cnn_time);
    ...
    midStepAccTime += elapsed_time(stepEnd, midStep);
    gettimeofday(&stepEnd, NULL);
    ...
    if (stepCount % 300 == 0 || stepCount == 1) {
      ...
      // printf("  Mean end time = %f\n\n", midStepAccTime / stepCount);
    }
#endif
