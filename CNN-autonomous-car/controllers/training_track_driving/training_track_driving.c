...
void load_stamps(char const* timeFileName, char const* velFileName, char const* angleFileName) {
...
int main(int argc, char **argv) {
  ...
  WbFieldRef car_trans_field = wb_supervisor_node_get_field(car_node, "translation");
  WbFieldRef car_rot_field = wb_supervisor_node_get_field(car_node, "rotation");

  // train track inital poses
  double CWinitialRotation[4] = {-0.01662080811297577, 0.024790612100833716, 0.9995544879041551, 0.403};
  double CWinitialTranslation[3] = {250.874, -323.838, 0.412048}; 
  double CCinitialRotation[4] = {0.006075581501692473, 0.0040733610068066015, 0.9999732471619052, -2.80453}; 
  ...
  while (wbu_driver_step() != -1) {
    ...
    // compute error in rotation between current car rotation and ref in angle file
    const double *car_rotation = wb_supervisor_field_get_sf_rotation(car_rot_field);
    ...
    double rot_error = rot_sign*car_rotation[3]+M_PI - (angle_stamps[curr_segm]+M_PI);
    int P = 1.5;
    ...
    if(mode == CW){
      snprintf(img_center, sizeof(img_center), "../../images/CW-images-test/center_%d.png", i);
      snprintf(img_left, sizeof(img_left), "../../images/CW-images-test/left_%d.png", i);
      snprintf(img_right, sizeof(img_right), "../../images/CW-images-test/right_%d.png", i);
    }
    else if(mode == CC){
      snprintf(img_center, sizeof(img_center), "../../images/CC-images-test/center_%d.png", i);
      snprintf(img_left, sizeof(img_left), "../../images/CC-images-test/left_%d.png", i);
      snprintf(img_right, sizeof(img_right), "../../images/CC-images-test/right_%d.png", i);
    }
