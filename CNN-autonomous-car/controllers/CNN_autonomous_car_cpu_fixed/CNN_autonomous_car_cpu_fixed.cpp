 ...
 * 		  fixed point and multi-threading version
 */
...
vector<fixed_point_t> forward_prop(vector<fixed_point_t> input){
	...
	for(int i = 0; i<net.nb_layers; i++){
		...
		if(layer_type == CONV){
			switch(layer_id){
				case 0: omp_set_num_threads(16);break;
				case 2: omp_set_num_threads(32);break;
				case 4: omp_set_num_threads(64);break;
			}
			...
			#pragma omp parallel
			{
				...
				for(int j = 0; j<net.linear_layers[layer_id].nb_neurons; j++){
					...
					for(int k = 0; k<net.linear_layers[layer_id].neurons[j].nb_weights; k++){
						sum += fixed_mul(net.linear_layers[layer_id].neurons[j].out_weights[k], prevx[k]);
					}

					if(i == net.nb_layers-1){
						x_private.push_back(sum + net.linear_layers[layer_id].neurons[j].bias);
					}
					else{
						x_private.push_back(elu(sum + net.linear_layers[layer_id].neurons[j].bias));
					}
				}
				#pragma omp for schedule(static) ordered
				for(int i=0; i<omp_get_num_threads(); i++) {
...
int main(void)
{
	...
	while (exit != -1) {
		...
		for(int i = 0; i < input_size/3; i++){
			...
			inputFixed.push_back(float_to_fixedpt((px_value - INPUT_MEANS[0])/INPUT_STDS[0]));
		}
		for(int i = input_size/3; i < 2*input_size/3; i++){
			int idx = 4 * ((floor((i-input_size/3)/320)) * (320) + ((i-input_size/3) % 320)) + 1;
			float px_value = input_char[idx];
			inputFixed.push_back(float_to_fixedpt((px_value - INPUT_MEANS[1])/INPUT_STDS[1]));
		}
		for(int i = 2*input_size/3; i < input_size; i++){
			int idx = 4 * ((floor((i-2*input_size/3)/320)) * (320) + ((i-2*input_size/3) % 320));
			float px_value = input_char[idx];
			inputFixed.push_back(float_to_fixedpt((px_value - INPUT_MEANS[2])/INPUT_STDS[2]));
		}
