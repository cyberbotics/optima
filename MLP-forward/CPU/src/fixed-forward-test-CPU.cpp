/**
 * Summary:
 *        Test speed of forward propagation on CPU with fixed-point
 * representation and multi-threading
 */
...
fixed_point_t sigma(fixed_point_t x) {
  ...
  // output = fmax(x,0.);
  output = float_to_fixedpt(
      tanh((float)x / (float)(1 << FIXED_POINT_FRACTIONAL_BITS)));
...
#pragma omp parallel for reduction(+ : nb_test_errors)
  ...
  for (int i = 0; i < nb_images; i++) {
    ...
    /*for(int j = 0; j < test_target_size; j++){
            printf("%d  %f\n",i*10+j, (float)get<1>(result)[1][j]/ (float)(1 <<
    FIXED_POINT_FRACTIONAL_BITS));
    }*/

    if (test_target[i][verify_classification(get<1>(result)[1])] < 0.5) {
      nb_test_errors++;
    }
  }
  printf("%d\n", nb_test_errors);
