...
#include "../tile-forward-test/mnist/include/mnist/mnist_reader.hpp"
#include "TileForwardProp.h"
...
const vector<int> netDimensions = {TileForwardProp_SIZE_LAYER_0, TileForwardProp_SIZE_LAYER_1, TileForwardProp_SIZE_LAYER_2};

// Input Parallelization
const int inVecSizeTile[] = {TileForwardProp_IN_VEC_SIZE1, TileForwardProp_IN_VEC_SIZE2};

// Tile blocks
const int blockSize[] = {TileForwardProp_TILE_OFFSET1, TileForwardProp_TILE_OFFSET2};
...
vector<vector<float>> load_biases(){
...
vector<vector<float>> compute_tile_weights(vector<vector<float>> linearWeights){
	vector<vector<float>> allWeights;

	int wXVec[] = {netDimensions[0]/inVecSizeTile[0], netDimensions[1]/inVecSizeTile[1]};

	// Organize weights
	for(int l = 0; l< netDimensions.size()-1; l++){
		vector<float> weights;
		for(int b = 0; b < ceil((float)netDimensions[l+1]/(float)blockSize[l]); b++){
			for(int i = 0; i < wXVec[l]; i++){
				for(int j = 0; j < blockSize[l]; j++){
					for(int k = 0; k < inVecSizeTile[l]; k++) {
						//printf("%d, %d, %d\n", l, b*blockSize[l]+j, i*inVecSizeTile[l]+k);
						if (l == netDimensions.size()-2 && j >= netDimensions[netDimensions.size()-2])
							weights.push_back(0.); // Padding
						else
							weights.push_back(linearWeights[l][(b*blockSize[l]+j)*netDimensions[l] + i*inVecSizeTile[l]+k]);
					}
				}
			}
		}
...
PropResult forward_prop_dfe(vector<vector<float>> input, int batchSize){
	...
	allBiases = load_biases();

	allWeights = compute_tile_weights(allWeights);
	...
	TileForwardProp_actions_t actions;
	...
	//actions.inmem_FHIDDENLAYER_KERNEL_mappedBias = allBiases[0].data();
	//actions.inmem_FOUTPUTLAYER_KERNEL_mappedBias = allBiases[1].data();
	...
	TileForwardProp_run(max_engine, &actions);
	struct timeval end;
	gettimeofday(&end, NULL);
	printf ("%f seconds\n", (end.tv_sec - start.tv_sec) + (end.tv_usec - start.tv_usec)*1e-6);

	printf("Unloading engine!\n");
...
{		
	...
	const string& folder = "src/tile-forward-test/mnist";
	...
	for (int i = 0; i< nb_images; i++){
		int padding = 2;
		vector<float>::const_iterator first = get<1>(result)[1].begin() + i * (test_target_size+padding);
		vector<float>::const_iterator last = get<1>(result)[1].begin() + (i+1) * (test_target_size+padding);
