...
using namespace std;
using PropResult =
    tuple<vector<vector<fixed_point_t>>, vector<vector<fixed_point_t>>>;

// Network dimensions
const vector<int> netDimensions = {FixedForwardProp_SIZE_LAYER_0,
                                   FixedForwardProp_SIZE_LAYER_1,
                                   FixedForwardProp_SIZE_LAYER_2};

// Parallel input
const int inVecSize[] = {FixedForwardProp_IN_VEC_SIZE1,
                         FixedForwardProp_IN_VEC_SIZE2};

// Parallel output
const int outVecSize[] = {FixedForwardProp_OUT_VEC_SIZE1,
                          FixedForwardProp_OUT_VEC_SIZE2};
...
vector<vector<u_int8_t>> split_input(int nbStreams, vector<u_int8_t> input) {
  ...
  for (int i = 0; i < nbStreams; i++) {
    vector<u_int8_t>::const_iterator first =
        input.begin() + i * splittedInputSize;
    vector<u_int8_t>::const_iterator last =
        input.begin() + (i + 1) * splittedInputSize;
...
PropResult forward_prop_dfe(vector<vector<u_int8_t>> input, int batchSize) {
  ...
  vector<vector<fixed_point_t>> s1(
      numEngines,
      vector<fixed_point_t>(firstLayerSize * batchSize / numEngines));
  vector<vector<fixed_point_t>> x1(
      numEngines,
      vector<fixed_point_t>(firstLayerSize * batchSize / numEngines));
  vector<vector<fixed_point_t>> s2(
      numEngines,
      vector<fixed_point_t>(secondLayerSize * batchSize / numEngines));
  vector<vector<fixed_point_t>> x2(
      numEngines,
      vector<fixed_point_t>(secondLayerSize * batchSize / numEngines));
  ...
  for (int i = 0; i < numEngines; i++) {
    ...
    printf("%d %d %d %d %d\n", batchSize / numEngines, batchFractionSize,
           zeroLayerSize, firstLayerSize, secondLayerSize);
...
int main(void) {
  ...
  vector<vector<float>> float_test_input =
      process_images(test_input_full, desired_nb_images);
  vector<vector<float>> float_test_target =
